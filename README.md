...
Aplicación web en Streamlit para buscar piensos directamente sobre un Excel de formulación, ordenarlos por aptitud según la consulta del usuario y usar FEDNA solo como contexto técnico informativo en el chat.
...
- Responde preguntas en lenguaje natural desde un panel de chat/consulta visible, editable y con 50 propuestas reutilizables y editables.
- Calcula un ranking de aptitud nutricional y devuelve el Top N usando el Excel como fuente de verdad para la búsqueda.
...
- Permite seleccionar varios piensos de toda la base de datos y compararlos por precio, nutrientes seleccionados e ingredientes de la fórmula.
...
1. **Búsqueda y ranking** hechos exclusivamente con los datos del Excel cargado.
2. **Recuperación de fragmentos** desde los PDF FEDNA cargados para contextualizar la respuesta del chat.

Esto permite separar claramente la recomendación de producto (Excel) del soporte técnico informativo (FEDNA).

## Uso del chat

La interfaz incluye:

- una lista visible con 50 propuestas editables de consulta
...
- selector multiselección para comparar varios piensos de toda la base de datos
...
## Cambio de estrategia de búsqueda

- El ranking de productos se calcula **solo con el Excel cargado**.
- Los PDFs FEDNA aportan **contexto técnico en el chat**, pero **no cambian el orden de los productos**.
- Si el chat incluye límites numéricos (por ejemplo `lisina >= 0.70`, `calcio entre 0.60 y 0.80`, `precio <= 290`), la app los aplica como **filtros duros** sobre el Excel.
- Si el chat pide prioridades como `proteína alta`, `energía alta` o `precio bajo`, la app ordena el Excel con esas prioridades.
- Si el chat contiene términos de nombre o fase del producto (por ejemplo `gestantes`, `acabado`, `crecimiento`), la app intenta filtrar por coincidencia textual en el nombre del pienso y en los ingredientes.
