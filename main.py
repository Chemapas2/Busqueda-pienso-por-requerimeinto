...
def default_app_readme() -> str:
    return """# Guía de uso de la app

Esta app tiene **dos tipos de análisis** y ambos trabajan sobre el **Excel cargado**.

## 1. Análisis de búsqueda y ranking

Este análisis sirve para responder una necesidad técnica escrita en lenguaje natural.

### Cómo funciona
- Se carga el Excel de formulación.
- Se elige la especie o categoría.
- Se seleccionan los nutrientes que quieres tener visibles o priorizar.
- En la caja de consulta se escribe una petición, por ejemplo:
  - `busca gestación con lisina > 0.70 y precio < 290`
  - `prioriza proteína alta y precio bajo`
  - `sin trigo y con calcio entre 0.80 y 1.10`
- La app interpreta esa consulta y aplica filtros sobre el Excel.
- Después ordena los productos con un **score de aptitud**.

### Qué devuelve
- Un **Top N** de piensos recomendados.
- Precio, aptitud y nutrientes visibles.
- Motivo resumido de la recomendación.
- Detalle del pienso seleccionado.
- Exportación a **Excel** del análisis completo.

### Para qué sirve
- Buscar rápidamente productos que cumplan unas condiciones nutricionales o económicas.
- Filtrar por nombre, nutrientes, ingredientes, límites y precio.
- Priorizar candidatos antes de una revisión técnica más fina.

## 2. Comparativa directa de piensos

Este análisis sirve para comparar varios piensos concretos entre sí, sin pasar por una consulta de ranking.

### Cómo funciona
- En la sección **Comparativa directa de piensos** se seleccionan uno o varios productos de la base de datos.
- La app construye una tabla comparativa con:
  - precio
  - nutrientes seleccionados
  - fórmula por ingredientes

### Qué devuelve
- Una tabla de comparación de nutrientes y precio.
- Una tabla de comparación de ingredientes por porcentaje de inclusión.
- Exportación a **Excel** de la comparativa.

### Para qué sirve
- Comparar productos que ya conoces.
- Ver diferencias rápidas entre fórmulas.
- Revisar si dos o más piensos son parecidos o claramente distintos.

## Diferencia entre ambos análisis

- **Búsqueda y ranking**: parte de una pregunta o criterio técnico y devuelve los productos que mejor encajan.
- **Comparativa directa**: parte de una selección manual de piensos y muestra sus diferencias.

## Formatos de Excel admitidos

La app intenta leer:
- formato tabular clásico, con una fila por pienso
- reportes tipo formulación con bloques `Specification:`
- reportes tipo formulación con bloques `SP:`
- secciones de ingredientes y análisis, incluidas variantes como `NUTRIENT ANALYSIS`

## Papel de FEDNA

FEDNA se usa como **contexto técnico informativo** en el chat.
No decide el ranking de los productos.
El ranking y la comparativa se calculan con los datos del Excel cargado.

## Exportaciones disponibles

### En el análisis de búsqueda y ranking
Se puede descargar un Excel con:
- resumen
- ranking
- notas y filtros
- detalle del pienso seleccionado
...
1. Carga el Excel.
2. Selecciona la especie.
3. Decide qué nutrientes quieres ver.
4. Usa primero el **análisis de búsqueda y ranking** para identificar candidatos.
5. Usa después la **comparativa directa** para revisar los piensos finalistas.

## Importante

La app ayuda a **filtrar, ordenar y comparar**.
La decisión final sigue siendo técnica y debe validarse con criterio nutricional, comercial y de uso práctico.
"""


def load_app_readme() -> str:
    candidates = [
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("Ayuda")
        st.checkbox("Mostrar README / guía de uso en la app", key="show_readme_in_app", value=False)
        st.download_button(
            label="Descargar README.md",
            data=load_app_readme().encode("utf-8"),
            file_name="README.md",
            mime="text/markdown",
            use_container_width=True,
        )

    if st.session_state.get("show_readme_in_app", False):
        with st.expander("README / guía de uso", expanded=True):
            st.markdown(load_app_readme())
